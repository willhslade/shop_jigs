...
End Function
Public Sub askmd5()
    Dim ttt As String, ttt2 As String, i As Long
    'ttt = InputBox("wtf")
    MsgBox md5("ТЕРм10-03-001-02Стойка, полустойка, каркас стойки или шкаф, масса, кг до 300 (шкаф 47U)")
    MsgBox md5("ТЕРм10-03-001-02Стойка, полустойка, каркас стойки или шкаф, масса, кг до 300 (Шкаф 47U)")
    
    ttt = "ТЕРм10-03-001-02Стойка, полустойка, каркас стойки или шкаф, масса, кг до 300 (шкаф 47U)"
    ttt2 = "ТЕРм10-03-001-02Стойка, полустойка, каркас стойки или шкаф, масса, кг до 300 (Шкаф 47U)"
    
    For i = 1 To Len(ttt)
        If Mid(ttt, i, 1) <> Mid(ttt2, i, 1) Then
            MsgBox i & ": " & Mid(ttt, i, 1) & " " & Mid(ttt2, i, 1)
        End If
    Next i
End Sub
Public Sub recalcmd5()
    Dim tt As String
    Dim tcell As Range
    For Each tcell In Selection.Cells
        tt = ActiveSheet.Cells(tcell.Row, 2).Value & ActiveSheet.Cells(tcell.Row, 3).Value
        tt = UCase(tt)
        tcell.Value = md5(tt)
    Next tcell
End Sub
